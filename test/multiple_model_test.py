...
class TestMultipleModel(unittest.TestCase):
    ...
    def test(self):
        model_path1 = "test/model/mobilenet_v2_1.0_224_inat_bird_quant.tflite"
        model_path2 = "test/model/efficientnet-edgetpu-M_quant.tflite"
        model_path3 = "test/model/dummy.tflite"
        cmd = f"edgetpu_compiler -a -o test/model/compiled "
        cmd += f"{model_path1} "
        cmd += f"{model_path2} "
        cmd += f"{model_path3} "
        cmd += f"{model_path3} "
